...
| Published Loom upload/TLS adoption | Complete locally; app release workflow pending | [Loom 1.25.0](https://github.com/gadicc/loom/releases/tag/v1.25.0) is published and pinned exactly, including private upload policy and verified runtime TLS; installed hashes match the reviewed release and postgres.js remains 3.4.9; frozen install, 3,922-test coverage, types, Biome, ordinary Loom check and an isolated production build pass; production Loom check still reports the pre-existing missing `db:migrate` app release workflow |
