...
own bucket and corresponding Vercel environment. These credential values are
not yet supplied or installed. An operator-only hidden-input helper is available
at `/tmp/magickli-r2-setup-proposal/enter-credentials.py`.
...
Relocation is an additional implementation milestone. The existing compatibility
reader and publication catalog still address the original bucket until the
verified relocation path is implemented and activated.
