...
/** A shared link restores its reading once; later changes stay local until the reader copies a new link. */
function GeomancyReading({ initial }: { initial: GeomancyReadingState }) {
  const ref = React.useRef<SVGSVGElement>(null);
  const [planetId, setPlanetId] = React.useState<PlanetId>(initial.planetId);
  ...
  const [houseNoStr, setHouseNoStr] = React.useState(initial.house);
  const [mothers, setMothers] = React.useState<TetraRow[]>(initial.mothers);
  ...
  return (
    <>
      <Container sx={{ my: 2, textAlign: "center" }}>
        ...
        <ExportControls
          target={ref}
          filename="astroGeomancy"
          link={{
            slug: "astro-geomancy-chart",
            props: { mothers: mothersToString(mothers) },
          }}
          share={() =>
            `/geomancy/reading?${readingSearchParams({
              mothers,
              house: houseNoStr,
              planetId,
            })}`
